...
import ru.antonov.trainticketservice.auth.dto.AuthRequestDto;
import ru.antonov.trainticketservice.auth.dto.AuthResponseDto;
import ru.antonov.trainticketservice.auth.dto.RefreshAccessTokenRequestDto;
import ru.antonov.trainticketservice.auth.entity.TokenMode;
import ru.antonov.trainticketservice.auth.entity.TokenType;
import ru.antonov.trainticketservice.common.exception.AuthenticationException;
...
import ru.antonov.trainticketservice.common.exception.RefreshTokenValidationFailureException;
...
public class AuthService {
    ...
    public AuthResponseDto refreshAccessToken(RefreshAccessTokenRequestDto request) {
        ...
        if (!jwtService.isTokenValid(refreshToken, TokenMode.REFRESH)) {
            throw new RefreshTokenValidationFailureException(
        ...
        User user = tokenService.findUserByToken(refreshToken)
                .orElseThrow(() -> new RefreshTokenValidationFailureException(
                        "Неуспешная валидация refresh токена",
