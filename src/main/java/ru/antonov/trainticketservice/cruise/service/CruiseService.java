...
import ru.antonov.trainticketservice.cruise.dto.DtoFactory;
import ru.antonov.trainticketservice.cruise.entity.Cruise;
import ru.antonov.trainticketservice.cruise.entity.Stop;

import ru.antonov.trainticketservice.cruise.repository.CruiseRepository;
...
public class CruiseService {
    ...
    public List<CruiseShortResponseDto> findAllCruisesWithFilter(CruiseFilterRequestDto request) {
        return cruiseRepository.findAllFilterByTargetStationsAndDateRange(
    ...
    public void updateCruiseStop(User principal, UUID cruiseId, UUID stopId, CruiseStopUpdateRequestDto request) {
        ...
        if (prevStopOpt.isPresent() && !prevStopOpt.get().getPlannedDeparture().isBefore(stop.getPlannedArrival())) {
            throw new BadRequestException(
                    String.format("Неправильно указано новое время прибытия. Коллизия: время прибытия на указанную" +
                                    " остановку - %s, время отправления с предыдущей остановки - %s",
                            request.getNewPlannedArrival(),
        ...
        if (nextStopOpt.isPresent() && !nextStopOpt.get().getPlannedArrival().isAfter(stop.getPlannedDeparture())) {
            throw new BadRequestException(
                    String.format("Неправильно указано новое время отправления. Коллизия: время отправления с" +
                                    " указанной остановки - %s, время прибытия на следущуюу остановку - %s",
                            request.getNewPlannedDeparture(),
